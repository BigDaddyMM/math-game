...
public class NumberComparisonGame2 : MonoBehaviour
{
    ...
    public void DisplayMathProblem()
    {
        ...
        // Ensure that numbers do not go below 0
        randomFirstNumber = Mathf.Max(0, randomFirstNumber);
        randomSecondNumber = Mathf.Max(0, randomSecondNumber);

        // Assign the first and second numbers
        firstNumberInProblem = randomFirstNumber;
        secondNumberInProblem = randomSecondNumber;

        // Compare if firstNumber is less than secondNumber
        bool isLessThan = firstNumberInProblem < secondNumberInProblem;

        // Assign answers based on the comparison
        answerOne = isLessThan ? 1 : 0;  // True
        answerTwo = isLessThan ? 0 : 1;  // False
